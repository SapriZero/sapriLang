...
// core/base/src/bucket/strategy.rs

// use crate::scanner::TokenStrategy;  // ← RIMUOVI QUESTA RIGA

/// Strategie di bucket sort
#[derive(Debug, Clone, Copy, PartialEq, Eq, Default)]
pub enum BucketStrategy {
    /// Strategia lineare: bucket proporzionali al valore
    Linear,
    /// Strategia esponenziale: bucket con crescita esponenziale
    Exponential,
    /// Strategia adattiva: sceglie automaticamente la migliore
    #[default]
    Adaptive,
}

/// Statistiche del bucket sort
#[derive(Debug, Clone, Default)]
pub struct BucketStats {
    pub n_buckets: usize,
    pub elements_per_bucket: Vec<usize>,
    pub min_value: f64,
    pub max_value: f64,
...
impl BucketStats {
    ...
    pub fn with_capacity(n_buckets: usize) -> Self {
        Self {
            n_buckets,
            elements_per_bucket: Vec::with_capacity(n_buckets),
            ..Default::default()
        }
    }
}

/// Strategia per token scanner (se serve, altrimenti rimuovi)
#[derive(Debug, Clone, Copy, PartialEq, Eq, Default)]
